...
export default function Home() {
  return (
    <motion.section 
      className="grid gap-8 py-10 lg:grid-cols-[1.2fr_0.8fr] lg:items-center"
      initial={{ opacity: 0, y: 10 }}
      animate={{ opacity: 1, y: 0 }}
      transition={{ duration: 0.35, ease: "easeOut" }}
    >
      <div>
        <span className="inline-flex rounded-full bg-emerald-500/15 px-3 py-1 text-sm font-semibold text-emerald-300 border border-emerald-500/20">BikeRoute Builder</span>
        <h1 className="mt-6 text-4xl font-bold tracking-tight text-slate-100 sm:text-5xl">Diseña, guarda y analiza rutas ciclistas con un mapa inteligente.</h1>
      ...
      <motion.div 
        className="rounded-2xl border border-slate-800 bg-slate-900/60 backdrop-blur-xl shadow-lg p-8"
        whileHover={{ y: -4, scale: 1.02 }}
        transition={{ type: "spring", stiffness: 250, damping: 20 }}
      >
        <h2 className="text-xl font-semibold text-slate-100">Funcionalidades clave</h2>
